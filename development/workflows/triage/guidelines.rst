...
-  *Good first issue*: the issue is *assumed* to be an easy one to fix, which makes
   ...
   resolves this issue. For details on how to assign this see
   :ref:`assigning good first issues <doc_good_first_issues>`.
...
- Anything :ref:`not appropriate for new contributors <doc_new_contributor_rules>`.
  Keep an eye out for refactors, issues that require new features to resolve,
  and stylistic changes or "nitpicking".
...
- Potentially controversial issues, issues that might need maintainer consensus or might involve
  significant discussion in review (this would include issues labeled ``discussion``).
- Issues that are not well understood or require testing to determine the scope or complexity
  (often labeled ``needs testing``) or that lack details or are unclear (usually labeled ``needs work``).
...
- Changes that are hard to validate or replicate, happening in edge cases or that only occur on certain hardware
...
- How complicated and smooth the review and merge process might be, and potential sources of friction. Consider especially
  ...
  Avoid issues where there's a risk of additional work if the solution is incomplete, or where there might be a lot of
  extra scrutiny to make sure it is accepted.

When possible, a good first issue should not only be an easy first task, but should also provide a smooth curve for introducing
the contribution process. Avoid overloading them with issues that might require figuring out a lot of different workflows at once,
or that might take a long time to get merged or involves a lot of unexpected work for the contributor.
...
At the same time, avoid assigning the label if you have doubts about it being appropriate. Make sure to remove it if things change.
