...
- Confirm that your :ref:`reproduction steps <doc_reporting_reproduction_steps>` are clear and reproduces your issue correctly.
- Add a :ref:`Minimal Reproduction Project (MRP) <doc_reporting_minimal_reproduction_project>` if needed.
- Make sure any necessary :ref:`video or screenshots <doc_reporting_video_and_screenshots>` are included and work correctly.
- Any :ref:`error messages or code snippets <doc_reporting_code_snippets>` are correctly formatted.
...
Please include the *full* crash report, especially the part after the "Dumping the backtrace." part. (Add note about building with debug)
...
Make sure to put an empty line before *and* after the link to any content. If you don't, GitHub won't embed the video or images,
and it will be much harder for others to view them.

.. note::

    Take screenshots or video on the actual device used; *do not* use another device or camera (such as a smartphone) to record the screen.
