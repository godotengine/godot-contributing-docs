...
- Check the documentation. A lot of issues are caused by users missing some details about how a feature works,
  or involve known limitations that are documented.
- Search for existing issues, open *and* closed, to reduce the risk of creating a duplicate issue.
  If you are unable to find a duplicate, that's OK. The triage team will deal with this if necessary,
  but searching for duplicates helps and is appreciated.
...
to ask in the `contributors chat <https://chat.godotengine.org/>`__ or the other `community channels <https://godotengine.org/community/>`__.
...
There are a few critical parts of an issue report that are critical, without which an issue is
...
Make sure to go through the steps yourself to confirm that they are correct. It can be easy to miss a step when
remembering what caused a problem.
...
.. TODO: Add note about screen recording software and not to use a camera to record the screen.
...
.. note::
    ...
    This section should not be taken as a list of required parts of an issue report, the section above describes what is and
